...
import { IProjectInterface } from "./PtojectsSection";
...
const icons: IconInterface[] = [
  ...
  {component: <RiOpenaiFill className={iconsStyle} />,name: "Prompt Engineering", }
];
...
export default function Project({project, position}: {
  ...
  return (
    <li className={` sm:flex items-center ${!position && "flex-row-reverse"}`}>
      <div className="self-start">
        <h2 className="font-semibold dark:text-white text-zinc-800">{project.title}</h2>
        <Image
          src={project.image}
      ...
      <div className="w-[500px] p-4 dark:text-white text-zinc-800">
        <p className="text-justify">{language === "EN"? project.descriptionEn : project.descriptionPt}</p>
        ...
        <ul className="flex relative gap-4 z-10">
